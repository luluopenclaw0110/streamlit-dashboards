...
with tab3:
    st.header("📋 沖繩之旅 7天6夜")
    st.caption("👨‍👩‍👧‍👦 盧家四口 + 👨‍👩‍👦 方家三口 · 2026年7月19日-25日")
    ...
    # Day 1
    st.markdown("#### Day 1 · 7/19 (六) · 出發日")
    col1, col2 = st.columns([1, 2])
    with col1:
        st.image("images/manzamo.jpg", width=300, caption="萬座毛")
    with col2:
        st.markdown("""
        - 🛫 **16:40** 台中起飛 → **19:10** 抵達沖繩
        - 🚗 領取租車
        - 🍖 晚餐：88牛排 / 拉麵
        - 🏨 入住：東京急Stay那霸（2晚）
        """)
    ...
    # Day 2
    st.markdown("#### Day 2 · 7/20 (日) · 那霸市區")
    col1, col2 = st.columns([1, 2])
    with col1:
        st.image("images/shuri.jpg", width=300, caption="首里城")
    with col2:
        st.markdown("""
        - 🏛️ 波上宮 + 波之上海灘
        - 🍜 En男爵和牛燒肉（午餐）
    ...
    # Day 3
    st.markdown("#### Day 3 · 7/21 (一) · 移動至BEB5")
    st.markdown("""
    - 🚗 退房，開車前往瀨良垣
    - 🏨 入住 BEB5 瀨良垣（4晚）
    - 🏖️ 下午：瀨良垣海灘
    ...
    # Day 4
    st.markdown("#### Day 4 · 7/22 (二) · 北部文化遊")
    st.markdown("""
    - 🏰 勝連城跡（世界文化遺產）
    - 🛒 AEON Mall（下午購物）
    - 🌅 傍晚：美國村看夕陽
    ...
    # Day 5 - 水族館日
    st.markdown("#### Day 5 · 7/23 (三) · 水族館日")
    col1, col2 = st.columns([1, 2])
    with col1:
        st.image("images/aquarium.jpg", width=300, caption="美ら海水族館")
    with col2:
        st.markdown("""
        - 🐠 美ら海水族館（世界最大黑潮水族館）
        - 🦈 黑潮之海：鯊魚、魟魚群
    ...
    # Day 6
    st.markdown("#### Day 6 · 7/24 (四) · 最後購物日")
    st.markdown("""
    - 🛍️ Outlet / 玩具反斗城
    - 🍜 琉風拉麵（午餐）
    - 🏖️ 把握最後海灘時光
    ...
    st.markdown("---")
    st.caption("🌺 圖片來源：維基百科 / Wikimedia Commons")
